...
class WTM:
    ...
    def train(self, train_data, batch_size=256, dir_alpha=0.001, learning_rate=0.001, test_data=None, num_epochs=100, is_evaluate=False, log_every=10, beta=1.0, ckpt=None):
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for iter, data in enumerate(data_loader):
                ...
                _, bows = data # used to be txt, bows
                bows = bows.to(self.device)

                bows_recon, theta_q = self.wae(bows) # outputs from WAE class' forward method
                
                theta_prior = self.wae.sample(dist=self.dist, dirichlet_alpha=dir_alpha, batch_size=len(bows), ori_data=bows).to(self.device)
                ...
                mmd = self.wae.mmd_loss(theta_q, theta_prior, device=self.device, t=0.1)# this becomes na
            ...
            if (epoch+1) % log_every == 0:
                save_name = os.path.join(ckpt_folder, f'epoch_{epoch+1}.ckpt')
                # f'WTM_checkpoints/{self.taskname}_tp{self.n_topic}_a{dir_alpha}_lr{learning_rate}_log_{epoch+1}.ckpt'
                checkpoint = {
        ...
        # plt.xlabel('epochs')
        # plt.legend()
        # plt.savefig(f'wtm_data/wlda_tc_scores.png')
    ...
    def get_embed(self,train_data, num=1000):
        self.wae.eval()
        data_loader = DataLoader(train_data, batch_size=32,shuffle=False, num_workers=1, collate_fn=train_data.collate_fn)
        embed_lst = []
        cnt = 0
        for data_batch in data_loader:
            _, bows = data_batch # used to be txt, bows
            embed = self.inference_by_bow(bows) # inference by bow, so probably don't need the lines
