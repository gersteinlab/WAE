...
class WAE(nn.Module):
    def __init__(self, encode_dims=[2000, 1024, 512, 20], decode_dims=[20, 1024, 2000], dropout=0.0, nonlin='relu'):
        ...
        self.bn_input = nn.BatchNorm1d(encode_dims[0])
    ...
    def decode(self, z, dropout=True): # dropout False when model evaluating
        hid = z
        for i, (_, layer) in enumerate(self.decoder.items()):
            hid = layer(hid)
            if i < len(self.decoder)-1:
                if dropout:
                    hid = self.dropout(hid) # dropout is random, this is where stochasticity comes from
                hid = self.nonlin(hid)
        return hid

    def forward(self, x):
        x_normed = self.bn_input(x)
    ...
    def sample(self, dist='dirichlet', batch_size=256, dirichlet_alpha=0.1, ori_data=None):
        if dist == 'dirichlet':
            # z_true = np.random.dirichlet(np.ones(self.z_dim)*dirichlet_alpha, size=batch_size) # returns NAs for small alpha
            z_true = np.random.default_rng().dirichlet(np.ones(self.z_dim)*dirichlet_alpha, size=batch_size) # doesn't return NAs
